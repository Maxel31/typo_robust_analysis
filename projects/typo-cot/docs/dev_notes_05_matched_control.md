...
生成キューは 25/25 設定完了 (`logs/exp5/queue_progress.json`, 最終更新
2026-07-15T05:50)。全設定 attempts=1 で再試行なし。

検証 (`scripts/exp5/summarize_prod.py` → `results/prod/exp5/matched_build_summary.{csv,json}`):

- 25 設定すべて embedding_enabled=true (5 変数版)。rebuttal 期 2 設定
  (gemma-3-4b-it × gsm8k/mmlu) も 5 変数統一版で included。
- スキーマ: 全 25 設定でアーカイブ LXT-4 の results.json とエントリキー完全一致。
  sample_id 重複なし。
- `perturbed_tokens != 4` のサンプル (最大 16 件/設定, mmlu 系) は
  データセット側と件数完全一致 = 短文設問でプール枯渇する設計由来
  (アーカイブ LXT-4 も同一サンプルで 0〜3 個)。欠陥ではない。
- gemma-3-1b-it_gsm8k の対応付け 1318/1319 はアーカイブ LXT-4 側が
  gsm8k_00055 を欠くため (こちらの生成は 1319 完備)。
- SMD (|SMD|≥0.25 の 2nd-priority 変数): zipf が 5 設定
  (最大 Llama-3.2-3B×csqa -0.52)、centrality が 6 設定 (最大 gemma-3-4b×arc
  +0.37)。char_len / split_increment は全 25 設定で |SMD|<0.1。
  class_match_rate は 24 設定で ≥0.99 (Mistral×csqa のみ 0.9896)。
  unmatched は mmlu 4 設定で ≤0.07% のみ。
  Mistral×mmlu_pro の applied_from_matched_rate=0.821 が最低値。

統計 (`scripts/exp5/analyze_matched_control.py` を 25 設定にループ →
`results/prod/exp5/mcnemar/{setting}.json` / `mcnemar_summary.csv`):

- risk_diff (LXT-4 の追加低下) > 0: 24/25 設定 (例外 gemma-3-1b-it×gsm8k -0.005)。
- clean 正解条件付き McNemar p<0.05: 22/25。非有意は Llama-1B×gsm8k (p=0.41) /
  gemma-1b×gsm8k (p=0.09) / gemma-1b×mmlu_pro (p=0.21) — いずれも低精度床付近。
- 無条件 McNemar p<0.05 は 11/25 (残りは同方向・非有意)。

GLMM (誤答 ~ 条件 + (1|item) + (1|設定)) は未実装 (次ステップ)。
