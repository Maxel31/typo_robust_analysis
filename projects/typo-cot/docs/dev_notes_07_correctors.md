...
- 残り 57 設定 (32,203 ペア) は `run_within_run_queue.sh rest` で実行
  (冪等)。

### 全 75 設定の最終結果 (2026-07-19 07:03 完走)

- **within-run flip 0/45,641 (0.00%)** — 全 25 設定 × 3 校正器で 0。
  全ペアで生成テキスト自体が byte 一致 (n_gen_identical = n)、生成失敗 0、
  シャード失敗 0 (phase1 18 + rest 57 = 75 設定)。
- 同一集合のクロスラン flip (参考ノイズフロア) = **9.56% (4,362/45,641)**。
  ベンチ別ではクロスラン flip は mmlu_pro (16〜20%) > gsm8k > mmlu >
  arc/csqa の傾向 (詳細は within_run_summary.json の rows)。
- 25 設定 × 3 校正器の全表は
  `results/prod/exp7/within_run/within_run_summary.{json,md}` (再生成:
  `uv run python scripts/exp7/aggregate_within_run.py --output ... --output_md ...`)。
  results/ は gitignore のため、正典の数値は本ノートの M3×B2 表 +
  本節のプール値とする。
- 論文への反映: 「byte-identical 復元 → flip 0%」は within-run 測定
  (45,641 ペア、75 設定) で厳密に成立。本番評価のクロスラン比較で
  byte-identical 集合に見える flip (〜9.6%) は全量が再現性ノイズ。
