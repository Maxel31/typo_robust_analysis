...
## 結果 (2026-07-19, n=150 × 6 設定, typo + semantic)

**主推定量は median に切替**: `s2_kl_recovery = 1 - KL_patched/KL_base` は下に非有界
(clean↔typo の KL_base が小さいと軽い摂動で大きな負値)。**平均は外れ値に汚染**され、Gemma プール
の平均「ピーク」は −0.60 だが median ピークは +0.52。機構は独立検証済 (fine cumulative(0,3) が
粗い window[0,3] とビット一致、sham median≈0、all_positions median=1.0000)。bootstrap median 95%CI は
早期ピークで 0 を除外。

- **注入層/読み出し完了点 (H8f-1)**: **支持 6/6**。median 単層ピーク = Gemma **L5** (l/L=0.147),
  Llama **L3–L4** (0.107–0.143), Mistral **L1** (0.031)。全て <0.2、事前予測帯 (Gemma 2–6, Mistral 0–2)
  を的中。立ち上がり L0、半減 ~L11–L14、~0 化 l/L~0.6–0.8。相対深さ重ね描きで 3 モデル共通の
  早期ピーク形状 (Mistral 最早 ~0.03 → Llama ~0.11–0.14 → Gemma ~0.15)。
- **形状 (H8f-2)**: **支持 5/6 プラトー** (Llama L1–5, Mistral L1–3, Gemma-mmlu L5–7)。
  Gemma-gsm8k のみ単層スパイク (L5) → スパイク枝 (より強い局在主張)。
- **累積 (H8f-3)**: **修正 (1/6)**。累積は早期飽和 (sat L1–L7, l/L≤0.21) するが cum/single 比 1.03–1.45
  (概ね ~1.0–1.2)。→ 反証枝: typo の書き込みは早期帯に集中し単層/少数層でほぼ捕捉、分散書き込みではない。
- **検証点ヌル (H8f-4)**: **修正 (0/6 厳密)**。L20 (l/L~0.6)/L26 (l/L~0.8) は幅1でも ~0
  (「幅3希釈」への反証は後期層で成立) だが **L14 (l/L~0.45, 中間) は median 0.12–0.30 を保持**。
  → 読み出し完了は緩やかな裾を持ち中間深度まで及ぶ (鋭い早期カットオフではない)。
- **noising 十分性 (H8f-5)**: **修正 (1/6)**。単層 noising は best±1 で KL 乖離の 28–48% を再現
  (Llama-mmlu のみ ≥50%)。→ 早期層は必要だが単独では不十分、残りは早期プラトーに分散。
- **A3(a) other_span**: **支持 6/6**。無摂動下流位置は median |回復| ≤0.06 (スパンは +0.5–0.6)。
  漏出コピーは効果を運ばず、摂動語スパンのみが座 = A3 攻撃への直接反証。
- **A3(b) all_positions**: **支持 5/6** (median=1.0000; Llama-mmlu はトークン数一致ペア無し=N/A)。
- **A3(c) semantic**: **混在/モデル依存**。Mistral 同形 (Pearson 0.80–0.86, ピーク一致) → 読み出し
  ダイナミクスは入力摂動一般、typo 固有寄与は LXT/Random 倍率差に局在。Llama は高相関 (0.84–0.90)
  だが semantic ピークが 3–4 層早い。Gemma は非同形 (0.14–0.43) = typo 固有の深さ構造。
- **A3(d) attn 最早期の負値**: 粗い付録から観察として引き継ぎ、解釈保留。

**総括**: 中核の読み出し完了点局在は確証 (H8f-1 6/6 予測深度的中, H8f-2 5/6, A3a 6/6, A3b 5/6)。
副次予測 3 件は事前登録の反証枝で修正 (早期単帯書き込み・中間深度への緩やかな裾・単層の必要十分性欠如)、
A3c はモデル依存。Fig.5 を粗い窓棒グラフから median 相対深さ×回復プロファイルへ格上げ。
