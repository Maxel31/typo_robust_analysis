...
## Llama-3.2-3B 再走の集計 (2026-07-18, CPU 統合ウェーブ) — モデル間一般化の完成

HF Hub 障害で未取得だった Llama-3.2-3B の 2 設定 (07-17 再走、`HF_HUB_OFFLINE=1`)
を、上記 4 設定と同一手続き (cells[] の s2_kl_recovery、null 除外後の
median / frac_pos、question_span × clean→pert 主指標) で集計。

### 実行サマリと整合性検証 (Llama 2 設定)

| 設定 | n_tasks | done | excluded | failed | 除外内訳 | セル/ペア |
|---|---|---|---|---|---|---|
| Llama-3.2-3B × gsm8k | 362 | 337 (lxt 183 / rnd 154) | 25 | 0 | no_typo_answer 22, prompt_boundary_mismatch 2, no_answer_divergence 1 | 180 (10窓) |
| Llama-3.2-3B × mmlu | 750 | 709 (lxt 421 / rnd 288) | 41 | 0 | no_trigger_clean 20, no_typo_answer 12, prompt_boundary_mismatch 9 | 180 (10窓) |

- **整合性 PASS**: 両設定でファイル数 = n_tasks (362 / 750)、done+excluded の
  実数が run_summary.json と完全一致、config_hash 不一致 0。全 done ペアで
  セル数 = 180 (28 層モデル → w=3 で 10 窓 × 3部位 × 3位置種別 × 2方向)。

### S2 KL recovery プロファイル (question_span, clean→pert, median)

residual の層窓プロファイル。Llama は 28 層のため最終窓は [27,28):

| 設定 (lxt4) | 0-3 | 3-6 | 6-9 | 9-12 | 12-15 | 15-18 | 18-21 | 21-24 | 24-27 | 27-28 |
|---|---|---|---|---|---|---|---|---|---|---|
| llama × gsm8k | .722 | **.723** | .591 | .495 | .349 | .227 | .195 | .144 | .062 | ≈0 |
| llama × mmlu | .723 | **.766** | .734 | .653 | .556 | .382 | .248 | .167 | .090 | ≈0 |

各設定×条件の最良セルと部位ごとの早期層:
...
### 3 モデル統合の層プロファイル表 (lxt4, residual, question_span, c2p, median)

窓は各モデルの層数準拠 (Gemma 34 層 = 12 窓 / Mistral 32 層 = 11 窓 /
Llama 28 層 = 10 窓)。最終 2 窓は全設定 ≈0 のため省略:
...
### LXT-4 vs Random-4 の効果比 (Llama)

| 設定 | KL_unpatched 平均 (lxt / rnd) | 比 | 最良セル median 回復 (lxt / rnd) | 比 |
|---|---|---|---|---|
| llama × gsm8k | 0.274 / 0.056 | **4.9×** | 0.723 / 0.516 | **1.4×** |
| llama × mmlu | 0.519 / 0.084 | **6.2×** | 0.766 / 0.464 | **1.7×** |

### 答え flip (副指標, baseline 分岐ペアのみ)

| 設定 | lxt4 分岐/done | rnd4 分岐/done | flip@residual\|0-3\|q_span (lxt / rnd) |
|---|---|---|---|
| llama × gsm8k | 3/183 (2%) | 3/154 (2%) | 0/3 / 1/3 (n 僅少で無意味) |
| llama × mmlu | 54/421 (13%) | 59/288 (20%) | **45/54 (83%)** / **44/59 (75%)** |

GSM8K の flip 系指標が本 regime で無意味である点も Gemma/Mistral と同型
(分岐 2% のみ)。MMLU の positive control (residual|24-27 の cot_suffix /
answer_span パッチ) は分岐ペアの 93〜100% を flip (既知の直接上書き挙動)。

### 判定: 「早期層 residual 局在」は Llama でも再現 — 3 モデル一般化が完成

1. **residual 早期層優位**: 最良セルは 4 条件すべて residual|[0,6) (median
   lxt4 0.72〜0.77 / rnd4 0.46〜0.52)、深さ方向に単調減衰し最終窓 ≈0。
   ピーク位置 [3,6) は Gemma ([3,9)) と Mistral ([0,6)) の中間で、
   3 モデルとも「早期 1/3 の層」に収まる。
2. **mlp は早期のみ正で residual に次ぐ** (0-3 窓 0.40〜0.62。Llama は
   3 モデル中で mlp|0-3 が最も高く residual との差が小さいが、順位は不変)。
3. **attn は ≈0 かつ最早期 [0,3) が一貫して負** (−0.17〜−0.20)。
4. LXT-4 は Random-4 の 4.9〜6.2 倍の分布乖離かつ回復率 1.4〜1.7 倍 —
   Mistral (4.8〜4.9× / 1.5〜1.6×) とほぼ同水準。
5. MMLU 分岐ペアの flip 逆転 (早期 residual 1 窓で 75〜83%) も
   Gemma/Mistral (69〜77%) と同水準以上。

→ **主結論「質問タイポの効果は早期層の摂動語スパン residual 表現に局在し、
そこへの 1 窓パッチで過半が打ち消せる」は 3 モデル × 2 ベンチ × 2 摂動条件の
全 12 条件で再現**。前節の制約 3 (2 モデル限定) は解消。

集計スクリプト: セッションスクラッチパッドの ad-hoc 集計 (読み取りのみ)。
手続きは前節と同一で、数値の再導出は
`results/prod/exp8/Llama-3.2-3B-Instruct_{gsm8k,mmlu}/{lxt4,rnd4}/*.json` の
`cells[]` から可能。
