...
### スモーク結果 (2026-07-18, gemma-3-4b-it×MATH n=16, GPU 3 via run_with_gpu.sh)

`validate_smoke_math.py` 全項目 PASS:

- スキーマ: default/fixed 両 run とも results.json / stats / _cot.pt 完備、16/16。
- 回答スパン: 32/32 ペイロードで boxed ターゲット検出
  (target≠-1, cot_token_end = answer_token_start−1, 回答部が CoT 範囲外)。
- flip 10/10 で default と fixed の token_scores が実際に相違、
  非flip 6/6 は default dir への symlink 再利用。
- プロンプト再構築: 全16サンプルで exp-10 ペイロードと n_tokens 一致。
- **規約変更診断**: default 再計算 vs exp-10 フォールバック .pt の top10 Jaccard
  min 0.176 / median 0.333 / max 0.538 — ターゲット規約 (最終トークン → boxed
  回答トークン) の影響は大きく、default 側再計算 (交絡排除) の設計判断を支持。
- 分析1周 (union 除外 + 追加除外 0 → n=16): ρ(J@10|R)
  default −0.439 / fixed −0.235, Δρ=+0.205 CI[−0.073,+0.569] p=0.136
  (n=16 の参考値。方向・スケールとも GSM8K 帯域内で異常なし)。
- 実行速度: 0.24 s/sample (gemma-3-4b, backward のみ)。本番 Σ2,708 backward の
  GPU 純計算は小さく、wall-clock はロック待ち支配。

### 本番キュー起動 (2026-07-18 02:53)

- `setsid nohup uv run --no-sync python results/prod_math/run_queue_math.py`
  (sid=1822405)。順序: gemma-1b → Llama-1B → gemma-4b → Llama-3B →
  Mistral-7B → Qwen-7B、各モデル default → fixed → CPU 分析 → Δρ 表随時更新。
