...
状態: 起動済み(`logs/exp2_smoke_core.log` →
`results/smoke/exp2/gemma-3-4b-it_gsm8k_core_smoke/`)。GPU ロック輻輳のため待機中。
合格線: k=4 で top_rc_unrestricted ≫ stratum_matched_random(事前予測の方向)。
CPU ドライラン(clean 正解先頭5件)では無制限 top4 = ['18','dollars','=','9'] 等
(数値・演算語優位)、層内マッチ = ['16','breakfast','-','4'](数値↔数値、
内容↔内容)で設計どおり。結果は完了後に追記。
