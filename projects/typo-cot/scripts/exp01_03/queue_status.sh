#!/usr/bin/env bash
# 実験1+3 キューの進捗を一覧表示する (読み取り専用)。
# 使い方: bash scripts/exp01_03/queue_status.sh
set -u
...
while IFS=$'\t' read -r name _rest; do
    ...
    if [ -f "$PROJ/results/exp01_03/$name/summary.json" ]; then
        done_n=$((done_n + 1))
...
echo "=== exp01_03 queue status ($(date -Is)) ==="
echo "list: $LIST"
