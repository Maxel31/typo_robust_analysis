...
def main() -> None:
    ...
    stats = {"total_seen": 0, "scored": 0, "skip_no_answer_pattern": 0, "errors": 0}
    ...
    for entry in entries:
        ...
        jaccard_vs_rc = None
        rc_path = run_dir / "importance_scores" / f"{sid}_cot.pt"
        if rc_path.exists():
            rc_data = torch.load(rc_path, map_location="cpu", weights_only=False)
            rc_ranking = rc_word_ranking_from_cot_pt(rc_data)
