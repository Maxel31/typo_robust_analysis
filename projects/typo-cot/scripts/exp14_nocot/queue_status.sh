#!/usr/bin/env bash
# 実験14 no-CoT キューの進捗を一覧表示する (読み取り専用)。
# 使い方: bash scripts/exp14_nocot/queue_status.sh
set -u
...
while IFS=$'\t' read -r name _rest; do
    ...
    [ "$name" = "name" ] && continue
    if [ -f "$PROJ/results/exp14_nocot/$name/DONE" ]; then
        done_n=$((done_n + 1))
...
echo "=== exp14_nocot queue status ($(date -Is)) ==="
echo "list: $LIST"
...
[ "$pending_n" -gt 0 ] && [ "$pending_n" -le 12 ] && printf 'pending: %s\n' "${pending_names[@]}"
