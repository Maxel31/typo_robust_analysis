...
def load_condition_records(
    ...
    baseline = _load_results_index(
        root / "outputs" / "baseline" / f"{model}_{benchmark}" / "results.json"
    )
    perturbed = _load_results_index(
        root / "outputs" / "perturbed" / f"{model}_{benchmark}_{out_suffix}" / "results.json"
    )
    with open(
        root / "datasets" / "perturbed" / f"{model}_{benchmark}_{ds_suffix}" / "perturbed_dataset.json"
    ) as f:
        dataset = json.load(f)
