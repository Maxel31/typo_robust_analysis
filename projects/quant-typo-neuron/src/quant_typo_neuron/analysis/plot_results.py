...
def plot_robustness_comparison(
    ...
    set_theme()
    clean = df[df["typo_type"] == "clean"].set_index(["model", "quant_method", "bits", "benchmark"])
    typo = df[df["typo_type"] != "clean"]

    if "accuracy" in clean.columns and "accuracy" in typo.columns:
        merged = typo.merge(
            clean[["accuracy"]],
            left_on=["model", "quant_method", "bits", "benchmark"],
            right_index=True,
    ...
    if not merged.empty:
        sns.boxplot(data=merged, x="quant_method", y="robustness_gap", hue="typo_type", ax=ax)
