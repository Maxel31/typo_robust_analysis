...
from typo_utils.eval.metrics import accuracy
...
def run_experiment(cfg: DictConfig) -> dict[str, Any]:
    ...
    metrics.update({
        ...
        "quant_method": cfg.model.get("quant_method", "none"),
    ...
    run_id = make_run_id(
        model=cfg.model.name,
        quant=cfg.model.get("quant_method", "none"),
        benchmark=bench.name,
        typo_type=typo_type,
        num_typos=num_typos,
    )
...
def _eval_log_likelihood(runner, bench, examples, few_shot_examples):
    ...
    all_choices = []
    for ex in examples:
        prompt = bench.format_prompt(ex, few_shot_examples)
        choices = bench.format_choices(ex)
        prompts.append(prompt)
        all_choices.append(choices)

    scores = runner.score_log_likelihood(prompts, all_choices)
