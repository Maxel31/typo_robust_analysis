...
class VLLMRunner:
        ...
        *,
        ...
        for prompt, conts in zip(prompts, continuations):
            for cont in conts:
                ...
                full_ids = tokenizer.encode(prompt + cont)
                prompt_token_count = len(tokenizer.encode(prompt))
                cont_logprobs: list[float] = []
                for j in range(prompt_token_count, len(prompt_logprobs)):
                    lp = prompt_logprobs[j]
                    if lp and j < len(full_ids):
                        token_id = full_ids[j]
                        if token_id in lp:
                            cont_logprobs.append(lp[token_id].logprob)
                        else:
                            cont_logprobs.append(
                                max(lp.values(), key=lambda x: x.logprob).logprob
                            )
                total = sum(cont_logprobs)
                if normalize and cont_logprobs:
                    total /= len(cont_logprobs)
                flat_scores.append(total)
                idx += 1
    ...
    def compute_perplexity(
        ...
        for text, out in zip(texts, outputs):
            ...
            total = 0.0
            count = 0
            for j, lp in enumerate(prompt_logprobs):
                if lp and j < len(text_ids):
                    token_id = text_ids[j]
                    if token_id in lp:
